...
#define MOVEGEN_addr (BYTE *)0xB000 // (~100 x 5 max.bytes)
#define MOVEHIST_addr (BYTE *)0xB500 // (~300 x 8 max.bytes)
                                                        //Carlsen plays 122 moves, btw.                                                 
struct chess_stru
 {
...
void push_genml(BYTE f, BYTE t, BYTE p, BYTE k, BYTE e)
        {
        Chess.gc++;
        *(mg_L++)=f; *(mg_L++)=t; *(mg_L++)=p; *(mg_L++)=k; *(mg_L++)=e;
        };
...
void pawngen() {        /* generates all pawn moves */
    ...
    if(Chess.w)
        ...
        if(mg_X<7 && mg_Y>0)
                {
                i = mg_sq-7;
                if( (Chess.B[i]&7) || (Chess.ep==i) ) addmove(mg_sq,i,r,0);
                }
        if(mg_X>0 && mg_Y>0)
                {
                i = mg_sq-9;
...
{
        Chess.gList = MOVEGEN_addr;
        Chess.mHist = MOVEHIST_addr;
